...
async def check_minecraft_player_auth_status(nickname: str, redis: RedisDep):
    ...
    if user_data_json_str:
        ...
        return {"status": True, "name": user_data.get("name"), "nickname": nickname, "course": user_data.get("course"), "faculty": user_data.get("faculty"), "nbook": user_data.get("nbook")}
...
async def set_student_auth_status(user: User, redis: RedisDep):
    ...
    data = {'name': user.name, 'course': user.course, 'faculty': user.faculty, 'nbook': user.nbook}
